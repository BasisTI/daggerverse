...
| `defaults.maven` | `sonar-plugin-version` | string | Versão do `sonar-maven-plugin`. Vazio usa o default fixo do módulo maven.
...
| `quality-type` | string
| Build system que analisa o *código* do target, quando ele difere de como a *imagem* é construída. Um de `maven`, `npm`, `uv` — nunca `dockerfile` nem `custom`, que não têm build system. Só é aceito junto de `sonar = true`. *Default: o próprio `type`.* Ver <<quality-type>>.
|===
...
| `sonar-plugin-version` | string | Sobrescreve `defaults.maven.sonar-plugin-version`.
...
. O `pom.xml` da raiz declara `<version>${revision}</version>` e a property `<revision>` com um valor de desenvolvimento local:
+
[source,xml]
----
...
Build e teste (`clean verify`), análise Sonar e publicação da imagem via plugin Jib. Suporta Maven Wrapper, cache do repositório local, daemon Docker opcional para Testcontainers, modo reactor e reporte de commit statuses.

Fora do modo reactor, a versão é aplicada com `versions:set`; no modo reactor, com `-Drevision`.

O scanner é invocado por coordenadas completas -- `org.sonarsource.scanner.maven:sonar-maven-plugin:<versão>:sonar` -- e não pelo prefixo `sonar:sonar`. O prefixo só resolve quando o projeto declara o plugin, já que ele não está nos pluginGroups padrão do Maven, e essa dependência implícita quebrava projetos que não são gerados pelo JHipster.

A versão é fixa (`sonar-plugin-version`, com default no módulo maven) por dois motivos: uma release nova da SonarSource não pode quebrar todas as pipelines de um dia para o outro sem commit em lugar nenhum, e o scanner precisa continuar compatível com o servidor, que sobe de versão no ritmo dele. Como consequência, o `<version>` declarado no pom deixa de influenciar qual scanner roda -- quem precisa de outra versão a declara no `ci/pipeline.toml`.
