...
O scanner é invocado por `org.sonarsource.scanner.maven:sonar-maven-plugin:sonar`, e não pelo prefixo `sonar:sonar`. O prefixo só resolve quando o projeto declara o plugin -- ele não está nos pluginGroups padrão do Maven -- e essa dependência implícita quebrava projetos que não são gerados pelo JHipster. Como a forma qualificada não fixa versão, um projeto que declare o `sonar-maven-plugin` continua usando a versão dele; quem não declara recebe a última release.
