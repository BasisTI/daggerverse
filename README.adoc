...
Like maven and npm, the module is path-aware: `module-path` says where the project sits inside the tree it was handed. The build receives the project directly and leaves it empty; the quality check receives the whole repository and fills it in, so that the `.git` directory reaches the scanner. Without SCM data SonarQube cannot tell which lines a merge request changed, marks the *entire* project as new code, and fails the quality gate on the target's historical debt -- which is what happened to every uv target until this was fixed.

For the same reason the scanner analyses the *source* tree rather than the build output: the build output is `/app` after `uv sync`, with `.venv` inside it. The build still runs before the analysis, so a broken `uv sync` fails the quality job.
